...
public class World{
  ...
  public void worldControl(){
    ...
    while (true){
      ...
      if (positionPiece != dimensionBoard) //verifico si es necesario crear la pieza
       ...
       /* re-evaluo las piezas que se muestran por pantalla.*/
         int warningGame = 0;

          while (warningGame < LIMIT_CAPSULE){
              if (!endOfGame(capsule[warningGame])) warningGame++;
              else{
  ...
  public int movePiece(){
    ...
    boolean answer = board.checkPositions(piece, positionBox , 0);
  ...
  public int scannerAll(int option){
    ...
    /* metodo que verifica si el usuario perdio y ya no puede jugar mas.*/

  public boolean endOfGame(Piece typePiece){
    int positionBox = 0;
    boolean checkingEndOfGame = true;

    while ((positionBox < board.numberOfBoxes) && (checkingEndOfGame)){
      if (!board.checkPositions(typePiece , positionBox , 1)) positionBox ++;
      else checkingEndOfGame = false;
    }

    return checkingEndOfGame;
  }
