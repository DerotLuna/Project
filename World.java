...
public class World{
  ...
  public World(Player player, byte dimension, byte idBoard){
    ...
    if (idBoard == 1){
      ...
      /*board.endOfGame(capsule[counterOfPieces]);*/
      }
    }
  ...
  public void worldControl(){
    ...
    while (true){
      ...
      for (byte counterOfPieces = 0; counterOfPieces < LIMIT_CAPSULE; counterOfPieces ++) {
        ...
        //puede que llame al verificar EndOfGame
     ...
     if (positionPiece != dimensionBoard) //verifico si es necesario crear la pieza
      ...
      /* "podria ser mi end of game"
        for (int positionPiece = 0 ; positionPiece < NUMBER_OF_PIECE ; positionPiece ++){
            board.endOfGame(capsule[positionPiece]);
       }
  ...
  public int scannerAll(int option){
    int userAnswer = 0; boolean flag = true; int maximun = dimensionBoard*dimensionBoard;
    while(flag == true){
      ...
      userAnswer = scanner.nextInt(); System.out.print("\n");
      if (option == 1 && (userAnswer < LIMIT_CAPSULE) && (userAnswer >= 0)) flag = false;
      else if (option != 1 && (userAnswer < maximun) && (userAnswer >= 0)) flag = false;
